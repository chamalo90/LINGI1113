...
#define RINGING 7 // State used when it's time to WAKE UP ! :)
/** END PROGRAM STATES **/
...
unsigned short alarmStatus; // Alarmclock ON / OFF
unsigned short alarmDelay; // used to count 30 sec of alarm ringing
unsigned short alarmCheck; // used to know if alarm already rang today.
char LCDPrint[32];
char h[3];
char m[3];
char s[3];
...
Fréquence obtenue par test 25,3 MHz
*****************/
void ConfigTimer0(void){
...
void UpdateTime(){
	if(milliSec>=1000){
		milliSec -= 1000; // if didn't update for some ms, don't set millisec to 0, just 
		seconds++;
		if(stateMenu == RINGING){ 
			SwitchLedState();
			alarmDelay--;
		}
		if(alarmDelay == 0){ // stops alarm & put settings on OFF, switch off leds.
				alarmDelay = 30;
				stateMenu = STANDARD_STATE;
				alarmStatus = 0;
				LED0_IO = 0;
...
/***************
* IsItTime
* Mets à jour l'état si l'heure d'alarme est l'heure courante.
***************/
void IsItTime(){
	if(alarmStatus & (hours == alarmHours) & (minutes == alarmMinutes) & stateMenu != SET_ALARMCLOCK_MINUTES & stateMenu != SET_ALARMCLOCK_HOURS & !alarmCheck){
		stateMenu = RINGING;
		alarmDelay = 30;
		alarmCheck = 1;
	}
...
/**************
Update LCD
Mets à jour l'affichage LCD en fonction de l'état courant.
*****************/
void UpdateLCD(){
	switch(stateMenu){
		case STANDARD_STATE: 
