...
PUBLIC zone_t find_n_contig_zone(
  ...
  int n_zones			/* number of zones needed */
	...
	unsigned b_chunk;
	bit_t bit, last_valid, start_map_block, n_block, zmap_size;
	int count = 0;
	...
	zmap_size = sp->s_zones - (sp->s_firstdatazone - 1); /* number of zones in zmap */
	...
	current_chunk = &block->b_bitmap[b_chunk]; /* load chunk where 'bit' is stored */
	...
	while(TRUE){
		...
		if(current_chunk_ptr == chunk_end){ /* end of chunk reached */
			...
			current_chunk = &block->b_bitmap[0];
		}		
	}
}
